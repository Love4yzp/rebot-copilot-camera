...
当前进度见 [`PROGRESS.md`](./PROGRESS.md)，原始设计与决策记录见 [issue #1](https://github.com/Love4yzp/rebot-copilot-camera/issues/1)。
改代码前先读 [`AGENTS.md`](./AGENTS.md) —— 那里有四条违反了不会报错、只会让结果错的铁律。
...
1. **新建序列** —— 左栏「+ 新建」。
2. **开示教** —— 底栏「开始示教」。臂**先保持握持不动**（没人扶住就松劲的臂会垂下去），你一推它，它测到运动就放开变成零力浮动。
3. **拖到位、松手、按一下** —— 停手约 0.25 秒后臂自动锁在那个位置，按「记录当前位置」存成一个点。重复。
