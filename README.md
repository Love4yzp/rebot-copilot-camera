...
1. 左栏「+ 新建」建序列。
2. 底栏「开始示教」。臂**先握持不动** —— 没人扶住就松劲的臂会垂下去。
3. **推它一下**，臂测到运动就放开，变成零力浮动，可以自由拖。
4. 拖到位，**松手**。停手约 0.25 秒后自动锁在那里。
5. 按「记录当前位置」。
6. 重复 3–5。
...
开始前对**整条序列**做限位与自碰撞预检，包括相邻两点之间的路径 —— 两个各自合法的点位，中间的直线可能穿过臂自己的底座。不合法直接拒绝，**臂一动不动**。
...
| 前端 3D 空白 | URDF / mesh 没加载 | submodule init 过没？`curl -I :18790/assets/urdf/00-arm-rs_asm-v3/urdf/00-arm-rs_asm-v3.urdf` 应返 200 |
