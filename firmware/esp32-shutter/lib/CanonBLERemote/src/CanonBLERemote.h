...
private:
    ...
    const BLEUUID STATUS_CHARACTERISTIC;
    ...
    BLERemoteCharacteristic *pRemoteCharacteristic_Status = nullptr;
    ArduinoNvs nvs;
...
public:
    ...
    // Diagnostics: the host drives these to iterate on the wire protocol
    // without re-flashing. None of them are part of the shutter path.
    bool writeControlBytes(const uint8_t *data, size_t length);
    bool armWithName();
    String readStatusHex();
    bool subscribeStatus();
    bool writeRawChar(const char *uuidStr, const uint8_t *data, size_t length);
    String readRawChar(const char *uuidStr);
