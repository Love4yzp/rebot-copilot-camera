...
### B8. 近零位 joint4 负折被判 link2×link5 自碰撞

**现状**：sim 示教捕获过一个名为「零点」的位姿（j4 ≈ -0.10 rad，其余关节 ≈ 0），模型判 `link2_0 collides with link5_0`：goto / 执行的预检拒绝（400，臂不动），capture 只警告不拒绝（设计如此——臂物理上就在那里，见 `app/backend/api/poses.py`）。真零位（全 0）合法。#8 的结构性排除不含这一对——link2 与 link5 非相邻、静止姿态不撞，所以它参与每一次校验。模型行为已用 `backend.safety.kinematics.validate_pose` 离线复核重现，sim 与 prod 走同一模型。

**待实测**：真臂在 j4 微负的近零构型下 link2 与 link5 是否真碰、间隙多少。若真碰，示教规范加一条「joint4 不向负侧折」；若只是网格余量保守，收窄对应碰撞几何后回填。验法：解除急停进示教，j4 从 0 缓慢向负侧掰，边看间隙边试 goto 触发校验的角度，两值一起记。
