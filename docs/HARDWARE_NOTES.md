...
**现状（2026-08）**：机上无相机；夹爪已装回并**接线**（官方一整套：电机 0x07 在总线，yaml `gripper: true`，7 关节，profile 锁 `gripper`，质量走 URDF 全值）。 读回的 `gripper` 位置与 `joint2` **逐位相等**（`0.008436… == 0.008436…`，2026-08-14 从 `/ws` state 实测）——夹爪反馈疑似串位：读路径 `get_state()` 逐个 `_motor_map[name].get_state()`，夹爪 motor_id 0x07 却报出 joint2 的值；夹爪暂不操作、先记录不追。
