...
| **上一个完成的** | 本 commit：**贡献指引 + 仓库收尾**——新增 `CONTRIBUTING.md`（贡献流程一条清单 + 架构体检：稳的地方标了守住它的测试、债的地方有 owner 和触发条件、会断的地方有真机清单，「够不够好」可独立验证）；删 session scratch（`.scratch/` 周会稿、`.zcode/plans/` 一份无关项目的计划文件）；`.gitignore` 加 `.zcode/`/`.scratch/`；AGENTS 文档分工表加 CONTRIBUTING 行。代码未动，474 仍绿。此前历史见 `git log`。 |
