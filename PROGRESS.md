...
| **进行中** | ① 编排交互迭代（站位 ⇄ 时间轴双密度）：C1（`6f616e6` 文档与手势急救）、C2（站档位）已落地；C3 时间轴缩放 + 拖入占位预览在途。② 配件区 UI 回归：配对相机 / 测快门 / 重新检测配件的按钮在前端替换时掉了（API 端点还在，组件无调用），配件区入口待设计。③ 真机验证 + `docs/HARDWARE_NOTES.md` 待实测项回填（见 🚧） |
| **上一个完成的** | 编排交互迭代 C2：TimelineView 重构为单一编辑器双密度（站档位默认组装 ⇄ 时间轴档精修，编辑逻辑一份、三条结构规则写进文件头）；新增 StationCard / StationConnector / TrackBlock 子组件与 `selection.ts` / `easing.ts` 共享文件；站卡 = 位姿名 + 停留步进器 + 动作 chips + 「＋ 动作」，选中带 ‹ › 重排与删除；密度页签「站位 ｜ 时间轴」localStorage 记忆；空序列三步引导；追加后站列自动滚到尾端 |
