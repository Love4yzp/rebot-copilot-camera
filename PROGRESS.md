...
| **上一个完成的** | 本 commit：**架构卫生一轮**——① 层级边界 AST 机器锁（`tests/test_layer_boundaries.py`：四条散文边界 executor 不 import 闩锁 / ActionContext 无 arm 句柄 / api 不直连校验器 / arm 不实现运动学 + ActionContext 字段白名单 + mode/resting 一致性不变量，mutation 验证咬合）；② `actions/shutter.py` → `shoot.py` 消同名异物（与驱动包 `shutter/` 区分）；③ `plugin.json` 加 `api_version` 闸门（配件期开工前的兼容保险，旧清单视为当前版本继续加载）；④ PROGRESS 减肥（进行中压缩、上一个完成清空——历史只活在 `git log`）；⑤ 代码地图给超前模块打 parked 标。474 绿 + ruff 绿 + 前端 build 绿。此前历史见 `git log`——commit message 比任何文档都详细 |
