...
| **进行中** | ① 编排交互迭代（站位 ⇄ 时间轴双密度）：C1 文档与手势急救已完成待提交；C2 站档位（TimelineView 密度重构）、C3 时间轴缩放 + 拖入占位预览在途。② 配件区 UI 回归：配对相机 / 测快门 / 重新检测配件的按钮在前端替换时掉了（API 端点还在，组件无调用），配件区入口待设计。③ 真机验证 + `docs/HARDWARE_NOTES.md` 待实测项回填（见 🚧） |
| **上一个完成的** | 编排交互迭代 C1：ARCHITECTURE.md / TIMELINE.md 双密度立场与词汇修订（站位 = 保持块 + 块内动作 + 出向过渡；保持块/事件标记退为工程词）；素材库位姿卡「＋追加」一步成站；选中块「＋ 动作」显性按钮（双击保留为快捷）；TeachBar「保存为站位」（推臂→一步成站）；Inspector 浮层化（开合不再挤压时间轴）；3D 视角禁止仰视（phi ≤ π/2） |
