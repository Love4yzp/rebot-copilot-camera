...
| **上一个完成的** | 进站（臂当前位姿 → 首站）一等公民化，两个 commit：后端 `c27d79e` —— execute 碰撞预检补上「当前位姿 → 首站」段（此前只查相邻位姿，goto 早有、execute 漏了）；进站限速 0.5 → 0.25 rad/s（对齐 demo 安全同步 15°/s，未标定记入 🚧 B5）；executor 新增 `approaching` 标志贯通 wire / mock / 契约。前端 —— 执行按距离分级：臂离首站 > 0.3 rad 时「执行」变「去起点」（复用限速 goto，到位停住后执行才回来），执行中首块进站显示「接近起点…」；预演从臂当前位姿起播进站段（时长镜像后端公式，匀速线性），拖播放头直接落计划尺、前摇取消 |
