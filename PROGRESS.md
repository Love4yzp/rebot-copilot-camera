...
| **上一个完成的** | `#84` fix: 固件第一次真编译（版本钉死 + `isConnected()` 不能当闸门 + `STATUS` 三态） |
| **备注** | **82/84 完成，321 个测试绿，ruff 干净，前端 TypeScript 编译通过。** 只剩 #6/#7 两个硬件实测（外加 #84 留下的一条上板确认） —— 没有臂就是做不了，不是没做。软件侧全部就绪：起服务后示教 → 录点 → 播放 → 急停 → 409 全程实测过（`uv run -m backend.app --sim`）。**上机第一件事**：`./manage.sh setup && ./manage.sh push`，然后看 `./manage.sh status` 报的是真臂还是模拟器；接着按 `docs/HARDWARE_NOTES.md` 的「待实测」段逐条填。挂相机后重点重调 `FloatLockConfig` 的速度阈值和 `ArmSession` 的 MIT 增益。 |
...
| 84 | L+E | fix: 固件第一次真编译（版本钉死 + `isConnected()` 不能当闸门 + `STATUS` 三态） | DONE | 固件写完之后**一次都没编译过**。`pio run` 暴露三件事：① `platform = espressif32` 不钉版本会解析到 55.x（Arduino core 3.x），佳能库 `pair()` 里两处 `BLEDevice::setEncryptionLevel` 在 core 3.x 已搬去 `BLESecurity` —— 编译错误报在库自己的源码里，读起来像库坏了而不像 core 挪了位置；钉在 `espressif32@6.11.0`（2.0.x core 线的最后一版），库也钉在 tag `#1.0.2`。② **`isConnected()` 不能当 `SHOOT` / `FOCUS` 的闸门** —— 这是读上游源码才看出来的：`CanonBLERemote::init()` 只从 NVS 读回相机地址，**不建立连接**，真正的连接由 `trigger()` / `focus()` 惰性发起。用 `isConnected()` 挡等于把唯一能建立连接的那次调用挡在门外，板子于是永远连不上，每帧回 `ERR camera not connected`，而相机就醒着摆在旁边。改成挡「有没有配对过」（`getPairedAddressString()`，与静态存储期零初始化出来的 `00:00:00:00:00:00` 比）。③ `PAIR` 原本把 `camera.pair()` 和 `camera.isConnected()` 写成同一次 `reply()` 的两个实参 —— C++ **不规定实参求值顺序**，编译器可以先读连接状态再执行配对，于是报出来的原因是一次扫描之前的事实；拆成局部变量。顺带把 `STATUS` 改成 `connected` / `disconnected` / `unpaired` **三态**：`unpaired` 要一个人拿着相机走菜单，`disconnected` 是配好的板子在两次拍摄之间的常态、下一帧自己会好，压成两态会把人指到错的地方。失败文案由「相机拒绝」改为 `camera unreachable` —— 库把特征值写下去就返回真、不等机身回话，「拒绝」是这条链路根本观测不到的状态。实测编译通过：RAM 13.5%、Flash 27.0%。**仍待上板**：重启后第一条 `SHOOT` 是否真的自己建链（慢几秒但成功），已登记进 `docs/HARDWARE_NOTES.md` 的待实测段 |
