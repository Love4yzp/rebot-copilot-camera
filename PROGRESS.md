...
| **进行中** | ① 编排交互迭代（站位 ⇄ 时间轴双密度）：C1（`6f616e6`）、C2（`a6b24eb` 站档位）、C3（缩放 + 拖入占位）全部落地。② 配件区 UI 回归：配对相机 / 测快门 / 重新检测配件的按钮在前端替换时掉了（API 端点还在，组件无调用），配件区入口待设计。③ 真机验证 + `docs/HARDWARE_NOTES.md` 待实测项回填（见 🚧） |
| **上一个完成的** | 编排交互迭代 C3：时间轴档缩放（− / ＋ / 适配 + Ctrl/⌘ 滚轮锚点缩放，刻度密度自适应；手势只读元素 rect，缩放感染不到手势代码）+ 拖位姿入轨的虚线占位预览（数据空间防振荡，既有块即时让位）；`model.ts` 导出 `DEFAULT_HOLD_S`；重排拖动不做占位（落点即所得，刻意从简） |
