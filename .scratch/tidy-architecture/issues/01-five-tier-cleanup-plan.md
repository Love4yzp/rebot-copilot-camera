...
（2026-08-17，操作者裁决：「按你的来」）全部照单。五档按票内方案执行；R0 四条铁律 + 机器校验网不动；R1 五档递进、每档独立 commit 全绿；R2 档 4 后置；R3 threejs 参考技能保留；R4 HTML 原型删；R5 `data/`；R6 契约 `seed:true` 用例 + 常量单一来源归入档 3；R8 `contract/` 分裂保持；R9 README 两版分区速查表。→ 05–09、11 解除阻塞。
