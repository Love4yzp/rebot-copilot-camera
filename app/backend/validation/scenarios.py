            ...
            self.command_rows.append(
                {
                    ...
                    "kind": "command",
                    "t": command.t,
                    "q": dict(command.q_des),
                    "v": dict(command.v_des),
                    "kp": dict(command.kp),
                    "kd": dict(command.kd),
...
def _peak_command_speed(rows: list[dict[str, Any]], joint: str = "joint2") -> float:
    pairs = [
        (r["t"], r["q"].get(joint)) for r in rows if r["kind"] == "command" and joint in r["q"]
    ]
    return max(
        (abs((q1 - q0) / (t1 - t0)) for (t0, q0), (t1, q1) in zip(pairs, pairs[1:]) if t1 > t0),
        default=0.0,
    )
...
def _run_later_transition() -> tuple[dict[str, Any], Rig]:
    ...
    sequence = Sequence(
        name="later-short-transition",
        blocks=[
            HoldBlock(pose_id=a.id, duration_s=0.0),
            TransitionBlock(duration_s=0.5),
    ...
    _run_until(
        rig,
        lambda: rig.controller.executor is not None and rig.controller.executor.is_finished,
        20.0,
    )
    executor = rig.controller.executor
    final_q = float(rig.transport.get_state()[0][1])
    return {
        "scenario": "later_short_transition",
        "peak_command_speed_joint2_rad_s": _peak_command_speed(rig.command_rows),
        "nonzero_desired_velocity": any(
            abs(row.get("v", {}).get("joint2", 0.0)) > 1e-9 for row in rig.command_rows
        ),
        "phase": executor.phase.value if executor else None,
        "final_arrival_error_rad": abs(final_q - 0.8),
    }, rig
...
def _run_gap(gap: float) -> tuple[dict[str, Any], Rig]:
    ...
    _run_until(
        rig,
        lambda: (
            rig.controller.executor is not None
            and rig.controller.executor.phase.value == "transition"
        ),
        1.0,
    )
    ...
    before = float(
        next(
            r["q"]["joint2"]
            for r in reversed(rig.command_rows)
            if r["kind"] == "command" and "joint2" in r["q"]
        )
    )
    ...
    executor = rig.controller.executor
    recovery_rows = [
        row for row in rig.command_rows[recovery_cursor:] if "joint2" in row.get("q", {})
    ]
    recovery = recovery_rows[0] if recovery_rows else {}
    recovery_q = recovery.get("q", {}).get("joint2")
    recovery_v = recovery.get("v", {}).get("joint2")
    return {
        "scenario": f"gap_{gap:g}s",
        "gap_s": gap,
        "actual_tick_gap_s": actual_tick_gap,
        "clock_plant_delta_s": rig.clock.t - rig.plant.t,
        "reference_before": before,
        "reference_after": after,
        "reference_jump_rad": after - before,
        "latch_absorbed": rig.latch.is_latched,
        "recovery_command_q": recovery_q,
        "recovery_command_v": recovery_v,
        "recovery_is_hold": bool(
            rig.latch.is_latched and recovery_q is not None and abs(recovery_v or 0.0) <= 1e-9
        ),
        "executor_terminated": executor is None,
        "post_gap_phase": executor.phase.value if executor else "aborted",
        "phase": executor.phase.value if executor else None,
    }, rig
...
def _run_wait() -> tuple[dict[str, Any], Rig]:
    ...
    _run_until(
        rig,
        lambda: rig.controller.executor is not None and rig.controller.executor.is_waiting,
        30.0,
    )
    executor = rig.controller.executor
    ...
    entry_rows = [
        row
        for row in rig.command_rows
        if row["kind"] == "command" and any(joint in row.get("q", {}) for joint in JOINTS)
    ]
    ...
    while rig.clock.t < wait_end - 1e-12:
        ...
        tick_rows = [
            row
            for row in rig.command_rows[before_tick:]
            if row["kind"] == "command" and any(joint in row.get("q", {}) for joint in JOINTS)
        ]
        if tick_rows:
    ...
    final_q = float(rig.transport.get_state()[0][1])
    stable = bool(wait_hold_targets) and all(
        target == wait_hold_targets[0] for target in wait_hold_targets[1:]
    )
    return {
        "scenario": "wait_transition_0.5_pause_5s",
        "phase_at_wait": phase_at_wait,
        "wait_start_s": wait_start,
        "wait_end_s": wait_end,
        "wait_tick_count": len(wait_tick_gaps),
        "wait_tick_gaps_s": wait_tick_gaps,
        "wait_command_ticks": wait_command_ticks,
        "wait_entry_hold_target": wait_entry_hold_target,
        "wait_hold_targets": wait_hold_targets,
        "wait_target_stable": stable,
        "commands_during_wait": held_commands,
        "resume_accepted": resumed,
        "phase_after_resume": executor.phase.value if executor else None,
        "error": executor.error if executor else None,
        "final_arrival_error_rad": abs(final_q - 0.8),
    }, rig
...
def _run_partial_noise() -> tuple[dict[str, Any], None]:
    ...
    return {
        "scenario": "partial_feedback_noise",
        "joint3_reference_change_rad": second.q_des["joint3"] - first.q_des["joint3"],
    }, None
...
def _run_retarget() -> tuple[dict[str, Any], Rig]:
    ...
    sequence = Sequence(
        ...
        blocks=[
            HoldBlock(pose_id=a.id, duration_s=0.0),
            TransitionBlock(duration_s=4.0),
            HoldBlock(pose_id=b.id, duration_s=0.0),
        ],
    )
    rig.controller.play(sequence, {a.id: a, b.id: b}, source="validation")
    ...
    old_row = next(
        r for r in reversed(rig.command_rows) if r["kind"] == "command" and "joint2" in r["q"]
    )
    ...
    sampled_rows = [
        row for row in rig.command_rows if row["kind"] == "command" and "joint2" in row["q"]
    ]
    sampled = sampled_rows[-1]
    return {
        "scenario": "midmove_retarget",
        "old_reference": old,
        "new_reference": new,
        "reference_jump_rad": new - old,
        "old_reference_velocity_rad_s": old_v,
        "new_reference_velocity_rad_s": new_v,
        "velocity_jump_rad_s": new_v - old_v,
        "sampled_reference_q_delta_rad": sampled["q"]["joint2"] - new,
        "sampled_reference_v_delta_rad_s": sampled["v"].get("joint2", 0.0) - new_v,
        "sampled_dt_s": sampled["t"] - new_row["t"],
        "measured_boundary_q_delta_rad": float(new_sample["q"][1] - old_sample["q"][1]),
        "measured_boundary_v_delta_rad_s": float(new_sample["v"][1] - old_sample["v"][1]),
    }, rig


def _source_hashes() -> dict[str, str]:
    from reBotArm_control_py import motion_profiles, physics

    root = Path(__file__).resolve().parents[2]
    paths = {
        "session": root / "backend/arm/session.py",
        "executor": root / "backend/core/executor.py",
        "controller": root / "backend/core/controller.py",
        "watchdog": root / "backend/safety/watchdog.py",
        "sdk_motion_profiles": Path(motion_profiles.__file__),
        "sdk_physics": Path(physics.__file__),
    }
...
def run_scenarios() -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    rigs: list[Rig] = []
    scenarios = (
        (_run_later_transition, "later_short_transition"),
        (lambda: _run_gap(0.1), "gap_0.1s"),
        (lambda: _run_gap(1.0), "gap_1s"),
        (_run_wait, "wait_transition_0.5_pause_5s"),
        (_run_partial_noise, "partial_feedback_noise"),
        (_run_retarget, "midmove_retarget"),
    )
    ...
    for rig in rigs:
        finite = all(
            np.isfinite(sample["q"] + sample["v"] + sample["tau"]).all()
            for sample in rig.plant.samples
        )
        ...
        physical.append(
            {
                ...
                "finite_state": finite,
                "sample_count": len(rig.plant.samples),
                "max_abs_q_rad": float(
                    max((np.max(np.abs(sample["q"])) for sample in rig.plant.samples), default=0.0)
                ),
                "max_abs_v_rad_s": float(
                    max((np.max(np.abs(sample["v"])) for sample in rig.plant.samples), default=0.0)
                ),
                "max_abs_tau_nm": float(
                    max(
                        (np.max(np.abs(sample["tau"])) for sample in rig.plant.samples), default=0.0
                    )
                ),
                "torque_saturation_samples": saturation,
                "contact_samples": sum(
                    int(sample.get("ncon", 0) > 0) for sample in rig.plant.samples
                ),
                "max_contacts": max(
        ...
        rows.extend(
            {
                ...
                "kind": "measured",
                "t": sample["t"],
                "q": dict(zip(JOINTS, sample["q"])),
                "v": dict(zip(JOINTS, sample["v"])),
                "tau": dict(zip(JOINTS, sample["tau"])),
            }
            for sample in rig.plant.samples
        )
        rig.close()
    return {
        "mode": "controller-sequenceexecutor-mujoco",
        "scenarios": results,
    ...
    with path.open("w", newline="", encoding="utf-8") as stream:
        ...
        writer.writerow(
            [
                ...
                "kind",
                "t",
                *[f"q_{n}" for n in names],
                *[f"v_{n}" for n in names],
                *[f"tau_{n}" for n in names],
            ]
        )
        for row in rows:
            writer.writerow(
                [
                    ...
                    row["kind"],
                    row["t"],
                    *[row.get("q", {}).get(n, "") for n in names],
                    *[row.get("v", {}).get(n, "") for n in names],
                    *[row.get("tau", {}).get(n, "") for n in names],
                ]
            )
...
def check_payload(payload: dict[str, Any]) -> list[str]:
    ...
    failures: list[str] = []
    expected_results = {
        "later_short_transition",
        "gap_0.1s",
        "gap_1s",
        "wait_transition_0.5_pause_5s",
        "partial_feedback_noise",
        "midmove_retarget",
    }
    ...
    if (
        abs(by_name["partial_feedback_noise"].get("joint3_reference_change_rad", float("inf")))
        > 1e-9
    ):
    ...
    if wait.get("wait_tick_count", 0) <= 0 or wait.get("wait_command_ticks", 0) != wait.get(
        "wait_tick_count", 0
    ):
    ...
    if (
        not isinstance(wait_entry_target, dict)
        or not isinstance(wait_targets, list)
        or not wait_targets
    ):
        ...
        if missing_joints or any(
            not isinstance(target, dict) or any(joint not in target for joint in JOINTS)
            for target in wait_targets
        ):
            failures.append("WAIT hold target is missing arm joint commands")
        elif any(
            target.get(joint) != wait_entry_target.get(joint)
            for target in wait_targets
            for joint in JOINTS
        ):
    ...
    if (
        wait.get("phase_after_resume") != "done"
        or wait.get("final_arrival_error_rad", float("inf")) > 0.01
    ):
    ...
    for name in ("gap_0.1s", "gap_1s"):
        ...
        if (
            abs(
                by_name[name].get("actual_tick_gap_s", float("inf"))
                - (by_name[name].get("gap_s", float("nan")) + APP_PERIOD)
            )
            > 1e-6
        ):
            failures.append(f"{name} injected gap did not match actual tick gap")
    if (
        not by_name["gap_1s"].get("latch_absorbed")
        or not by_name["gap_1s"].get("recovery_is_hold")
        or by_name["gap_1s"].get("post_gap_phase") not in ("aborted", None)
    ):
        failures.append("1s gap did not enter holding safe lock")
    retarget = by_name["midmove_retarget"]
    required_retarget_fields = (
        "reference_jump_rad",
        "velocity_jump_rad_s",
        "sampled_reference_q_delta_rad",
        "sampled_reference_v_delta_rad_s",
        "sampled_dt_s",
    )
    missing_retarget_fields = [
        field
        for field in required_retarget_fields
        if field not in retarget
        or not isinstance(retarget[field], (int, float))
        or isinstance(retarget[field], bool)
        or not math.isfinite(retarget[field])
    ]
    if missing_retarget_fields:
        failures.append(
            f"midmove retarget has missing or non-finite fields: {missing_retarget_fields}"
        )
        return failures
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run optional headless motion validation scenarios"
    )
    ...
    payload.update(
        {
            ...
            "settings": {
                "seed": SEED,
                "app_period_s": APP_PERIOD,
                "physics_dt_s": PHYSICS_DT,
                "integrator": "implicitfast",
                "urdf_sha256": hashlib.sha256(assets.urdf_path().read_bytes()).hexdigest(),
