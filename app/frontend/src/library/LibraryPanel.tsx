 ...
 *
 * Selecting a pose does not move the arm; goto is an explicit button.
 */
  ...
  return (
    <aside className="lib" aria-label="素材库">
      ...
      <button
        ...
        disabled={gotoLocked || !poses.some((p) => p.id === selectedPose)}
        onClick={() => {
      ...
      {collapsed ? null : onUseTemplate && tab === "templates" ? (
        ...
        <div
          className={`lib__pane ${poses.length === 0 ? "lib__pane--empty" : ""}`}
        >
          {poses.map((pose) => {
            ...
            return (
              <div
                key={pose.id}
                className={`lib__pose${selectedPose === pose.id ? " lib__pose--selected" : ""}`}
                role="button"
                tabIndex={0}
                aria-pressed={selectedPose === pose.id}
                onKeyDown={(event) => {
                  if (
          ...
          <button
            type="button"
            className={
              poses.length === 0 ? "lib__record primary" : "lib__record ghost"
            }
        ...
        <Dialog label="删除位姿" onClose={() => setConfirmDelete(null)}>
          <div className="sheet__head">
            <h2 className="sheet__title">
              删除位姿「{confirmDelete.pose.name}」？
            </h2>
          </div>
          <p className="hint">
            正被 {confirmDelete.links.count}{" "}
            条序列用着。删掉后那些序列执行前会被拒绝。
          </p>
