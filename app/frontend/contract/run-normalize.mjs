 ...
 * frontend dependency, so no new tooling) and prints the normalizer's canonical
 * transcript as JSON on stdout.
 *
 * Usage: node app/frontend/contract/run-normalize.mjs [cases-dir]   (from anywhere)
 *
 * `tests/test_contract.py` shells out to this and diffs the transcript against
 * the FastAPI TestClient's. Run it by hand to see the normalizer's half on its own.
 */
...
const built = buildSync({
  entryPoints: [path.join(here, "normalize-driver.ts")],
  bundle: true,
  platform: "node",
  format: "esm",
  write: false,
