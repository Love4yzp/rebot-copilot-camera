...
def make_playing_sequence(client: TestClient, hold_s: float = 30.0) -> str:
    ...
    client.patch(
        f"/api/sequences/{sid}",
        json={"blocks": [{"type": "hold", "pose_id": pose, "duration_s": hold_s, "markers": []}]},
    )
            ...
            "blocks": [
                {
                    ...
                    "pose_id": pose,
                    "duration_s": 5.0,
                    "markers": [{"kind": "wait", "params": {}, "at": 1.0, "estimate_s": 0.0}],
                }
            ]
        },
    )
...
def test_websocket_streams_seq_playback_progress(rig):
    ...
    client, controller, arm, clock = rig
    pose = client.post(
        "/api/poses", json={"name": "p", "joints": {"joint1": 0.3, "joint2": 0.0}}
    ).json()["id"]
    sid = client.post("/api/sequences", json={"name": "ws"}).json()["id"]
    client.patch(
        f"/api/sequences/{sid}",
        json={"blocks": [{"type": "hold", "pose_id": pose, "duration_s": 0.3, "markers": []}]},
    )
...
def test_the_service_arm_moves_without_anything_stepping_it(tmp_path: Path):
    ...
    pid = client.post(
        "/api/poses", json={"name": "sim", "joints": {"joint1": 0.3, "joint2": 0.0}}
    ).json()["id"]
