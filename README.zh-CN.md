...
第一次接手代码，先用 **[30 分钟看懂代码库](./docs/START_HERE.md)** 建立心智模型，再按任务读取专项文档。
...
开 **http://127.0.0.1:18790**。模拟臂会响应示教拖动（「+ 录位姿」进示教后，在监视器 3D 视图点一下臂的连杆选中关节，拖出现的圆环推它）、前往各站位、模拟触发快门 —— 整个流程都能完整走通。`./dev.sh status` 看端口上是谁。命令列表以 `./dev.sh --help` 为准。
...
2. **推它一下**，臂测到运动就放开，变成零力浮动，可以自由拖。模拟器里没有手 —— 在监视器 3D 视图点一下臂的连杆选中关节，拖出现的圆环来推。
...
| | |
|---|---|
| [docs/START_HERE.md](./docs/START_HERE.md) | 30 分钟代码库导览 |
